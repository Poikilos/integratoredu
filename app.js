...
function get_table_path_if_exists_else_null(section, filedb_name, category, create_enable) {
	var result = null;
...
function get_next_transaction_index(section, category, ymd_array, increment_after_getting_enable) {
	var table_path = get_table_path_if_exists_else_null(section, "transactions", category, false);
	var result = 0;
	if (table_path !== null) {
...
function get_table_entry(section, category, number) {
	var result = null;
	var parent_path = get_table_entry_parent_path(section, category, number, false);
	if (parent_path !== null) {
...
function get_table_entry_parent_path(section, category, number, create_enable) {
	var result = null;
	var table_path = get_table_path_if_exists_else_null(section, "tables", category, create_enable);
	if (table_path!==null) {
...
function get_table_entry_paths(section, category) {
	var table_path = get_table_path_if_exists_else_null(section, "tables", category, false);
	var results = [];
	if (table_path !== null) {
...
function get_table_entry_numbers(section, category) {
	var table_path = get_table_path_if_exists_else_null(section, "tables", category, false);
	var results = [];
	if (table_path !== null) {
		var millions = fun.getVisibleDirectories(table_path);
...
function get_next_table_index(section, category, increment_after_getting_enable) {
	var table_path = get_table_path_if_exists_else_null(section, "tables", category, false);
	var result = 0;
	if (table_path !== null) {
...
var hbs = exphbs.create({
		...
		show_billing_cycle_preview: function(section, username, selected_number, opts) {
			var ret = "";
			var mode = "reports";
			...
			if (user_has_section_permission(username, section, mode)) {
				if (fun.is_not_blank(selected_number)) {
					if (has_setting(section+".default_groupby")) groupby = peek_setting(section+".default_groupby");
					if (fun.is_not_blank(groupby)) {
						...
						if (cen_entry && cen_entry.end_dates && fun.is_blank(error)) {
							...
							if (has_setting(section+".autofill_requires."+groupby)) {
								primary_key_components = peek_setting(section+".autofill_requires."+groupby);
							}
							var ymd;
							var sff_i;
							var sff_len;
		...
		show_reports: function(container_enable, section, username, years, months, days, selected_year, selected_month, selected_day, selected_number, section_report_edit_field, opts) {
			...
			if (user_has_section_permission(username, section, mode)) {
				...
				ret += get_year_month_select_buttons(section, mode, username, years, months, selected_year, selected_month)+"\n";
				...
				if ( has_setting(section+".extended_hours_hourly_price") ) {
					...
					if (!selected_field) {
						...
						if (!selected_field) {
							if (has_setting(section+"."+mode+".selected_field_default")) {//else {
								selected_field = peek_setting(section+"."+mode+".selected_field_default");
								//var selected_field_msg = "null";
								//if (selected_field) selected_field_msg = selected_field;
					...
					if (selected_field) {//section_report_edit_field.hasOwnProperty(section)) {
						...
						//if (!peek_setting(section+"."+mode+".selected_field_default")) {
						//	console.log("  setting "+mode+".selected_field_default to "+selected_field);
						//	poke_setting(section+"."+mode+".selected_field_default", selected_field);
						//}
						//ret += "Selected Field:";
						...
						if (table_path !== null) {
										...
										if (fs.statSync(item_path).isFile()) {
											if (item_path.endsWith(".yml")) {
												try {
													...
													if (!dat[section][selected_year][selected_month][this_day][item_key].hasOwnProperty("tmp"))
														dat[section][selected_year][selected_month][this_day][item_key].tmp = {};
													dat[section][selected_year][selected_month][this_day][item_key].tmp.date_from_path = selected_year + "-" + selected_month + "-" + this_day;
													if (!items_by_date.hasOwnProperty(dat[section][selected_year][selected_month][this_day][item_key].tmp.date_from_path))
														items_by_date[dat[section][selected_year][selected_month][this_day][item_key].tmp.date_from_path] = [];
													items_by_date[dat[section][selected_year][selected_month][this_day][item_key].tmp.date_from_path].push(dat[section][selected_year][selected_month][this_day][item_key]);
							...
							for (var item_i=0, items_len=items.length; item_i<items_len; item_i++) {
								...
								if (item_enable && user_has_section_permission(username, section, mode)) {
									...
									//console.log("[ == ] this "+this_date+" "+this_time+"...");
										if (items_by_date.hasOwnProperty(items[item_i].tmp.date_from_path)) {
											this_date_items = items_by_date[items[item_i].tmp.date_from_path];
											...
											for (var inner_i=0,inner_len=this_date_items.length; inner_i<inner_len; inner_i++) {
												...
												if ((!this_date_items[inner_i].hasOwnProperty("active")) || fun.is_true(this_date_items[inner_i].active)) {
													...
													var prev_time = fun.get_time_or_stated_time(this_date_items[inner_i]);
													var prev_date = fun.get_date_or_stated_date(this_date_items[inner_i]);
													var this_time = fun.get_time_or_stated_time(items[item_i]);
													//var this_date = fun.get_date_or_stated_date(items[item_i], "item# "+item_i+"&"+prev_i);
													var this_date = fun.get_date_or_stated_date(items[item_i], "item# "+item_i+" & inner "+inner_i);
															...
															if (fun.is_not_blank(this_date) && fun.is_not_blank(prev_date)) {
																if ((prev_time!==null) && (this_time!==null)) {
																	...
																	if ((this_is_after_school==prev_is_after_school)) { //either before or after, as long as same
																		...
																		for (var ff_i=0; ff_i<ff_len; ff_i++) {
																			/*
																			if (fun.is_blank(items[item_i][section_form_fields[section][ff_i]]) || 
																				(fun.safe_equals_ci(items[item_i][section_form_fields[section][ff_i]], items[prev_i][section_form_fields[section][ff_i]])) || 
																				(
																					((typeof items[item_i][section_form_fields[section][ff_i]])=="string") && 
																					((typeof items[prev_i][section_form_fields[section][ff_i]])=="string") &&
																					(items[item_i][section_form_fields[section][ff_i]].trim().toLowerCase() == items[prev_i][section_form_fields[section][ff_i]].trim().toLowerCase()) )
																			) {
																				//console.log(" [ == ] "+items[item_i][section_form_fields[section][ff_i]] + " is " + items[prev_i][section_form_fields[section][ff_i]]);
																				match_count++;
																			}
																			//else console.log(" [ <> ] "+items[item_i][section_form_fields[section][ff_i]] + " is not " + items[prev_i][section_form_fields[section][ff_i]]);
																			*/
																			if (fun.is_blank(items[item_i][section_form_fields[section][ff_i]]) || 
																				(fun.safe_equals_ci(items[item_i][section_form_fields[section][ff_i]], this_date_items[inner_i][section_form_fields[section][ff_i]])) || 
																				(
																					((typeof items[item_i][section_form_fields[section][ff_i]])=="string") && 
																					((typeof this_date_items[inner_i][section_form_fields[section][ff_i]])=="string") &&
																					(items[item_i][section_form_fields[section][ff_i]].trim().toLowerCase() == this_date_items[inner_i][section_form_fields[section][ff_i]].trim().toLowerCase()) )
																			) {
																				//console.log(" [ == ] "+items[item_i][section_form_fields[section][ff_i]] + " is " + this_date_items[inner_i][section_form_fields[section][ff_i]]);
																				match_count++;
																			}
																			//else console.log(" [ <> ] "+items[item_i][section_form_fields[section][ff_i]] + " is not " + this_date_items[inner_i][section_form_fields[section][ff_i]]);
																		}
																		if (match_count>=ff_len) {
																			//console.log("matched "+match_count+" of "+ff_len);//+JSON.stringify(section_form_fields[section]));
																			//dup_index = prev_i;
																			dup_index = inner_i;
																			break;
																		}
										...
										else console.log("ERROR: "+items[item_i].tmp.date_from_path+" is not in items_by_date");
									}
								}
								...
								for (ssf_i=0; ssf_i<ssf_len; ssf_i++) {
									...
									if ((dup_index>-1) && (ssf_i===0)) {
										...
										ret += '  <input type="hidden" name="duplicate_key" id="duplicate_key" value="'+items[dup_index].key+'"/>'+"\n";
										ret += '  <input type="hidden" name="duplicate_time" id="duplicate_timef" value="'+items[dup_index].time+'"/>'+"\n";
										ret += '  <input type="hidden" name="duplicate_date" id="duplicate_date" value="'+fun.get_date_or_stated_date(items[dup_index],"field for "+item_i)+'"/>'+"\n";
										if (fun.visual_debug_enable) ret += '  <button class="btn btn-warning" type="submit">Mark dated '+fun.get_date_or_stated_date(items[item_i])+' <br/>Duplicate of '+fun.get_date_or_stated_date(items[dup_index],"button for "+item_i)+'<br/>(actual dup '+items[dup_index].key+': '+items[dup_index].ctime.substring(0,10)+') '+items[dup_index].time+'</button>'+"\n";
										else ret += '  <button class="btn btn-warning" type="submit">Mark as Duplicate of '+fun.get_date_or_stated_date(items[dup_index],"button for"+item_i)+' '+items[dup_index].time+'</button>'+"\n";
										//Mark '+item_i+':'+fun.get_date_or_stated_date(items[item_i],"button for "+item_i)+' Duplicate of '+fun.get_date_or_stated_date(items[dup_index],"button for"+item_i)+' (actual '+dup_index+': '+items[dup_index].ctime.substring(0,10)+') '+items[dup_index].time+'
										ret += '</form>';
									}
									...
									if (item_enable) {
										...
										if (fun.is_blank(item[column_name])) {
											if (has_setting(section+"."+mode+".suggest_missing_required_fields_enable")) {
												if (fun.is_true(peek_setting(section+"."+mode+".suggest_missing_required_fields_enable"))) {
						...
						if (user_has_section_permission(username, section, "billing")) {
							...
							if (table_path !== null) {
								...
								if (y_i===y_i) { //only not equal to itself if NaN
									...
									var cycle_entry_numbers = get_table_entry_numbers(section, category);
									...
									if (cycle_entry_numbers) {
										...
										for (var cen_i=0,cen_len=cycle_entry_numbers.length; cen_i<cen_len; cen_i++) {
											...
											if (cen!=selected_number) {
												var cen_entry = get_table_entry(section, category, selected_number);
												var cen_name = "";
												if (cen_entry && fun.is_not_blank(cen_entry.cycle_name)) cen_name = cen_entry.cycle_name;
										...
										if (fun.is_not_blank(selected_number)) {
											...
											//var s_sub_path = get_table_entry_parent_path(section, category, selected_number, false);
											//if (s_sub_path!=null) {
			...
			if (has_setting(section+".history_sheet_fields")) {
				...
				if (fields !== null) {
					...
					for (i=0, len=objects.length; i<len; i++) {
						ret += "<tr>"+"\n";
						if (force_date_enable) ret += "<td>"+fun.get_date_or_stated_date(objects[i], "history item "+i)+"</td>\n";
						for (field_i=0; field_i<f_len; field_i++) {
...
app.get('/', function(req, res){
	...
	res.render('home', {user: req.user, section: section, runme: req.session.runme, mode: mode, selected_setting: req.query.selected_setting, prefill: req.session.prefill, missing_fields: req.session.missing_fields, prefill_mode: prefill_mode, selected_year:selected_year, selected_month: selected_month, selected_day: selected_day, selected_number: selected_number, section_report_edit_field: req.session.section_report_edit_field, selected_item_key: selected_item_key, sections: user_sections, modes_by_section: user_modes_by_section, user_selectable_modes: user_selectable_modes, years: years, months: months, days: days, objects: items, this_sheet_field_names: this_sheet_field_names, this_sheet_field_friendly_names: this_sheet_field_friendly_names});
	...
	if (req.hasOwnProperty("user") && req.user.hasOwnProperty("username")) {
		...
		if (section && req.body.selected_year && req.body.selected_month) {// && req.body.selected_field) {
			if (user_has_section_permission(req.user.username, section, "modify")) {
				var table_path = get_table_path_if_exists_else_null(section, "transactions", "student", false);
				if (table_path !== null) {
	...
	if (req.hasOwnProperty("user") && req.user.hasOwnProperty("username")) {
		...
		if (user_has_section_permission(req.user.username, section, "billing")) {
			...
			if (matching_vars_count===0) {
				...
				//console.log(msg);
				//req.session.notice = msg;
				//push_next_transaction(section, category, ymd_array, record, as_username, autofill_enable)
				//push_next_table_entry(section, category, record, as_username, autofill_enable)
				...
				var results = push_next_table_entry(section, category, record, req.user.username, false);
				if ('error' in results) req.session.error = results.error;
			}
		}
		else {
			console.log(msg);
